# Implementation Tracker

Checklist for all implementation tickets. Mark each `[ ]` as `[x]` when complete.

## Per-ticket workflow

1. Implement ticket and update any documentation that needs updating based on the feature
2. Update this file and tick the checkbox for this ticket
3. Generate commit message → `commit-message.md`, wait for review before committing

---

- [ ] [01 — Database Migrations & Query Helpers](implementation/01-db-migrations-and-helpers.md)
- [ ] [02 — Supabase Auth Integration](implementation/02-supabase-auth.md)
- [x] [03 — OAuth Token Encryption](implementation/03-oauth-token-encryption.md)
- [x] [04 — Instagram OAuth Provider Client](implementation/04-instagram-oauth-client.md)
- [ ] [05 — API Auth Middleware & CRUD Routes](implementation/05-api-crud-routes.md)
- [ ] [06 — Queue Service: Content Sync Worker](implementation/06-content-sync-worker.md) (superseded by [queue-infrastructure.md](implementation/queue-infrastructure.md))
- [ ] [07 — Queue Service: Token Refresh Worker](implementation/07-token-refresh-worker.md) (superseded by [queue-infrastructure.md](implementation/queue-infrastructure.md))
- [ ] [08 — Portfolio SSR App](implementation/08-portfolio-ssr.md)
- [ ] [09 — Dashboard App](implementation/09-dashboard-app.md)
- [ ] [10 — Marketing Site](implementation/10-marketing-site.md)
- [ ] [11 — Shared Packages (shared-be, shared-fe)](implementation/11-shared-packages.md)
- [ ] [12 — Staging Infrastructure & Deployment](implementation/12-staging-infra.md)